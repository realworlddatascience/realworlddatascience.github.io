...
description: |
  Real World Data Science speaks with Julia Lane, professor at the NYU Wagner Graduate School of Public Service, about an initiative that seeks to improve public understanding of how datasets are used and the value they provide. 
categories:
  - AI
  - Data
  - Machine learning
...
It's difficult to put a figure on how much is spent each year on collecting and publishing data. According to the World Bank, it is [“painfully hard to obtain”](https://blogs.worldbank.org/opendata/how-much-should-governments-spend-data) information just on government spending on data, never mind other bodies and organisations. But investment in data likely constitutes a sizeable annual sum -- somewhere on the order of hundreds of billions of dollars would seem a reasonable (rough) estimate.

Now, here's an even more challenging figure to pin down: How much value does all this data provide? By and large, there is very little data -- or systematic collection of data -- on dataset usage. There’s no easy way to find all the users of a particular dataset and to see how the data has been used, or what research topics it may have contributed to.

Writing in the [Harvard Data Science Review in April 2022](https://hdsr.mitpress.mit.edu/pub/g6e8noiy/release/2), [Julia Lane](https://julialane.org/) and others explained that “the current approach to finding what data sets are used to answer scientific questions ... is largely manual and ad hoc.” They went on to argue that, “Better information on the use of data is likely to have at least two results: (i) government agencies might use the information to describe the return on investment in data sets and (ii) scientists might use the information to reduce the time taken to search and discover other empirical research.”
...
It emerged from three things, really, and I can trace the start of it back to 2016, when I was asked to build a secure environment that could host confidential microdata in order to inform the work of the [Commission on Evidence-Based Policymaking](https://obamawhitehouse.archives.gov/omb/management/commission_evidence).^[The Commission on Evidence-Based Policymaking was “charged with examining all aspects of how to increase the availability and use of government data to build evidence and inform program design, while protecting privacy and confidentiality of those data.”]

It was pretty clear that one of the recommendations that was going to come out of this commission was going to be a national secure data service. So, they asked me to lead on this because I had built the [NORC remote access Data Enclave at the University of Chicago](https://www.norc.org/services-solutions/data-enclave.html) 10 years before that.

So, this was the first step towards Democratizing Data.
...
So, step 1 was build secure environment. Step 2 was build capacity and identify the questions that are of interest to agencies so that they would put data into the secure environment.
...
That’s right. There were three different models that that were developed, and each one picks up different aspects of how authors mention data in publications, and all three have been extremely useful. We learned a lot about the variety of ways in which researchers cite the data that they use.
...
I think it is more of a sociotechnical issue than a technical one. We have the plumbing, but really what we need to do to figure out the incentives for researchers. We need to build a community around the data, which is what's happened with code and the sharing of code on platforms like GitHub.
...
::: {.g-col-12 .g-col-md-6}
How to cite
: Tarran, Brian. 2023. "Democratizing Data: Using natural language processing and machine learning to capture dataset usage." Real World Data Science, March 12, 2024. [URL](https://realworlddatascience.net/viewpoints/interviews/posts/2024/03/12/democratizing-data.html)
:::
:::
:::